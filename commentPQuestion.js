...
export default function commentPQuestion({ navigation, route, props }) {
    ...
    useEffect(() => {
        ...
        firebase.firestore().collection('Questions').doc(postCSection).collection('COMMENTS').onSnapshot(documentSnapshot => {
            const list = [];
    ...
    const liked = (doc, userUID) => {
        const user = firebase.auth().currentUser;
        const DocID = doc;
        firebase.firestore().collection('Questions').doc(postCSection).collection('COMMENTS').where('docRef', '==', DocID).where('LikedBy', 'array-contains', user.uid).get().then((snapshot) => {
            if (snapshot.empty) {
                addLike(DocID, userUID)
            } else {
    ...
    const addLike = (docIDForGood, otherUID) => {
        const user = firebase.auth().currentUser;
        const increment = firebase.firestore.FieldValue.increment(1);
        firebase.firestore().collection('Questions').doc(postCSection).collection('COMMENTS').doc(docIDForGood).update({
            likes: increment
        })
        firebase.firestore().collection('Questions').doc(postCSection).collection('COMMENTS').doc(docIDForGood).update({
    ...
    const addComment = () => {
        ...
        var docRef = firebase.firestore().collection('Questions').doc(postCSection).collection('COMMENTS').doc();
        docRef.set({
            LikedBy: [],
                        ...
                        <Card key={item.id} style={{ flexDirection: 'row', margin: 5 }}>
                            ...
                            <View style={{ flexDirection: 'column', justifyContent: 'flex-end' }}>
                                ...
                                <View style={{ flexDirection: 'row', marginTop: 30 }}>
                                    ...
                                    <Ionicons name="ios-heart-empty" size={27} color="green" style={{ marginRight: 15 }} onPress={() => liked(item.id, item.uid)} />
                                </View>
                            </View>
                        </Card>
                        : null
